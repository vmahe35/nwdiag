...
clean:	## Remove all generated files
	...
	-sudo rm -rf dist
	-sudo rm -rf src/nwdiag_nocher.egg-info
...
env:	## Create and activate a local virtual dev env
	...
	source $(VENV_NAME)/bin/activate && pip install --upgrade pip && pip install -r requirements.txt && pip install -e .

VERSION := $(shell nwdiag --version)

build: env	## Generate distribuable archives
	#python -m hatch build
	source $(VENV_NAME)/bin/activate && python setup.py install

run: env
	source $(VENV_NAME)/bin/activate && python src/run.py examples/nwdiag/simple.diag
